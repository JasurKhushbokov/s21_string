...
// Fixed memmove_4: Prevents reading beyond str1's allocated memory
START_TEST(memmove_4) {
  char str[] = "Hello, world!";
  char str1[] = "AB";  // At least 2 characters to copy safely
  s21_size_t len = 2;
  ck_assert_str_eq(memmove(str, str1, len), s21_memmove(str, str1, len));
}
END_TEST
...
// Fixed memmove_7: Correctly copies the null character
START_TEST(memmove_7) {
  char str[] = "Hello, world!";
  char str1[] = "\0";
  s21_size_t len = 1;  // Explicitly copy the null character
  ck_assert_str_eq(memmove(str, str1, len), s21_memmove(str, str1, len));
}
END_TEST
